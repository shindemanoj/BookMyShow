(function() {
	angular.module("BookMyShow").controller("userProfileController",
	...
	function userProfileController($location, UserService, $routeParams,
		...
		vm.getReviewsOfUser = getReviewsOfUser;
		...
		function getReviews(userId) {
			var promise = UserService.getReviews(userId);
		...
		function getReviewsOfUser(userId) {
			var promise = UserService.getReviews(userId);
			promise.success(function(response) {
				vm.userReviews = response;
			}, function(error) {
		...
		function getUsersFollowed(userId) {
			var promise = UserService.getUsersFollowed(userId).success(
					function(response) {
		...
		function getFollowers(userId) {
			var promise = UserService.getUsersFollowers(userId).success(
					function(response) {
		...
		function getTickets(userId) {
			var promise = UserService.getTickets(userId);
			promise.success(function(response) {
				vm.tickets = response;
			}, function(error) {
